...
public class RegistroAlmocoJob implements org.quartz.Job{
	public RegistroAlmocoJob() {
	...
	public void execute(JobExecutionContext context) throws JobExecutionException {
		...
		JobKey jobKey = context.getJobDetail().getKey();
		  ...
		  System.out.println("Executando verificação de marcação de inicio intervalo em: "+f2.format(d.getTime()));
		  try {
				  ...
				  while(rs.next()) {
					  ...
					  if(rs2.next()) {
						  if(rs2.getString("tipo_registro").equals("Entrada")) {
							  if(con.Inserir_simples("INSERT INTO registros (id_sistema,empresa,usuario,latitude,longitude,data_dia,distancia,datetime_mobile,datetime_servlet,hora,minutos,tipo_registro,local_registro,tipo_local_registro,site_operadora_registro,mes) VALUES ('1','1','"+rs.getString("id_usuario")+"','0','0','"+f2.format(time)+"',0,'"+f3.format(d.getTime())+"','"+time+"',"+aux_hora+","+aux_min+",'Inicio_intervalo','AUTOMÁTICO MSTP','AUTOMÁTICO MSTP','-',"+(d.get(Calendar.MONTH)+1)+")")) {
								  System.out.println("Registro de Inicio de intervalo inserido para usuário : "+rs.getString("id_usuario"));
							  }
							}else if(rs2.getString("tipo_registro").equals("Inicio_intervalo"))  {
								System.out.println("Registro de inicio de intervalo ok para usuário : "+rs.getString("id_usuario"));
							}
						  
					  }
				  }
				  time = new Timestamp(System.currentTimeMillis());
				  System.out.println("Encerramento do registro automático de inicio de intervalo em - "+f3.format(d.getTime()));
			  }else {
				  time = new Timestamp(System.currentTimeMillis());
				  System.out.println("Encerramento do registro automático de inicio de intervalo em - "+f3.format(d.getTime()));
			  }
